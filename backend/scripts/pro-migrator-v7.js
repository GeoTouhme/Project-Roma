...
const parseProCSV = (filePath) => {
    ...
    // FIND CORRECT INDICES FOR DESCRIPTION FIELDS
    const descIndex = headers.indexOf('Description');
    const aiDescIndex = headers.indexOf('Descreptions'); // Note the typo in CSV header 'Descreptions'
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (row['Upc']) {
            // Priority: Pro Description -> AI Description -> Empty
            let finalDesc = (parts[descIndex] || parts[aiDescIndex] || "").trim();
            finalDesc = finalDesc.replace(/^"|"$/g, '').replace(/""/g, '"'); // Clean quotes
...
const migratePro = async () => {
    await connectDB();
    // Wipe current partial import to start fresh with descriptions
    await Product.deleteMany({});
    console.log('🧹 Database cleared for fresh start with descriptions.');

    try {
        console.log('📖 Reading Professional CSV with Description fix...');
        ...
        console.log(`\n✨ FINISHED. Total with Descriptions: ${processed}`);
        process.exit(0);
