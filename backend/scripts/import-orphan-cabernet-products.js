/**
 * import-orphan-cabernet-products.js
 *
 * Creates placeholder products for orphan cabernet wine photos
...
async function run() {
    ...
    // Get category
    const wineCat = await Category.findOne({ name: 'WINE' });
    if (!wineCat) {
    ...
    function scanDir(dir, subdir) {
        ...
        for (const ent of entries) {
            ...
            if (ent.includes('(1)')) continue;

            const base = path.parse(ent).name.replace(/\s*\(\d+\)$/, '').trim();
            const stripped = stripLeadingZeros(base);
            if (dbCodes.has(stripped)) continue;

            orphanList.push({ upc: base, stripped, filename: ent, subdir, photoPath: full, category: wineCat, type: 'Wine' });
        }
    }
    scanDir(PHOTOS_DIR, '');
    ...
    for (let i = 0; i < orphanList.length; i++) {
        ...
        if (existing) {
            console.log(`${progress} Already exists: "${existing.name}" (code=${existing.code})`);
    ...
    for (let i = 0; i < productsToUpload.length; i++) {
        ...
        if (!fs.existsSync(o.photoPath)) {
            console.log(`${progress} Photo not found: ${o.photoPath}`);
    ...
    const reportPath = path.join(__dirname, 'orphan-cabernet-import-report.json');
