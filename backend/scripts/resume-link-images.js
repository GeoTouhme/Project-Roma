...
const LOG_PATH = path.resolve('/tmp/link-images-resume.log');
...
async function main() {
  ...
  // Re-check which items still need images in case a prior run partially completed
  const toUpload = [];
  for (const item of results.matched) {
    const product = await Product.findOne({ _id: new mongoose.Types.ObjectId(item.product_id) }, { projection: { images: 1, name: 1 } });
