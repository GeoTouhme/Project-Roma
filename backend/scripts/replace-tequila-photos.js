/**
 * replace-tequila-photos.js
 *
 * Replaces product photos in the database with new photos from the tequila-photos directory.
 * Matches photos by UPC code (filename) to Product.code field.
 * Recursively scans subdirectories (e.g., cocktail/).
 *
 * Usage:
 *   node scripts/replace-tequila-photos.js --dry-run    # Preview matches only, no changes
 *   node scripts/replace-tequila-photos.js              # Execute the migration
 *
 * Server paths:
 *   Photos: /usr/src/app/tequila-photos/
 *   Script: /usr/src/app/scripts/replace-tequila-photos.js
 */
...
const LOG_FILE = path.join(__dirname, 'tequila-photo-migration-log.json');
...
/**
 * Recursively get all .jpg files under a directory.
 * Returns array of relative paths from PHOTOS_DIR.
 */
...
const runMigration = async () => {
    ...
    if (!fs.existsSync(PHOTOS_DIR)) {
        ...
        console.error('   Example: docker cp /tmp/tequila-photos/ balport-backend:/usr/src/app/tequila-photos/');
        process.exit(1);
    }

    // Recursively read all photo files
    const allFiles = getAllJpgFiles(PHOTOS_DIR);
    console.log(`\n📁 Found ${allFiles.length} .jpg files in ${PHOTOS_DIR} (recursive)`);

    // Extract valid UPCs
    const photoMap = new Map(); // UPC -> relative path
    const skippedFiles = [];

    for (const file of allFiles) {
        const basename = path.basename(file);
        const upc = extractUPC(basename);
        if (upc) {
            // If duplicate UPC already exists, keep the first one found
            if (!photoMap.has(upc)) {
                photoMap.set(upc, file);
            } else {
                skippedFiles.push(file + ' (duplicate UPC: ' + upc + ')');
            }
        } else {
    ...
    for (const [upc, filename] of photoMap) {
        ...
        try {
            ...
            const tempPath = path.join(PHOTOS_DIR, `_temp_${path.basename(filename)}`);
